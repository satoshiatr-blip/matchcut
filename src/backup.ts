// 端末内データ（選手名簿・試合設定）のバックアップ・復元。
// 動画本体（数GB）は対象外——大きすぎて1ファイルに詰め込めないので、復元後は動画を選び直す前提
const KEYS = ['soccer-highlight:project', 'matchcut:roster'] as const
const APP = 'matchcut'
...
export function exportBackup() {
  ...
  a.href = url
  a.download = `matchcut-backup_${new Date().toISOString().slice(0, 10)}.json`
  a.click()
...
export async function importBackup(file: File): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  if (payload.app !== APP || !payload.data) return { ok: false, error: 'MATCHCUTのバックアップファイルではありません' }
  for (const k of KEYS) {
